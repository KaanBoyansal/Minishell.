...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/30 12:54:48 by mboyansa          #+#    #+#             */
/*   Updated: 2023/10/30 12:54:49 by mboyansa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_env_var(char *str, char **env, int *i, int *j)
{
	...
	*i += index;
	...
	while (*env)
	{
		if (ft_strncmp3(str, *env, len) == 0 && (*env)[len] == '=')
			break ;
		env++;
	}
	if (!(*env))
...
char	*ft_strdup3(char *str, int *start, char end)
{
	...
	len = ft_strlen2(str, end, *start) - *start;
